    ...
    const getdata = async () => {
      try {
        const res = await fetch(
          "https://reva-health-bd.herokuapp.com/api/v1/doctor/all?uid=sdjkhb"
        );
            ...
            <tbody className=" divide-y divide-gray-200 ">
              {data.map((item, i) => (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-3 py-1.5 text-sm text-gray-800 ">
                    {item.availability ? (
                      <button className=" bg-red-700 text-white py-2 px-5 rounded-md">
                        No&nbsp;
                      </button>
                    ) : (
                      <button className=" bg-green-700 text-white font  py-2 px-5 rounded-md">
                        Yes
                      </button>
                    )}
                  </td>
